`timescale 1ns / 1ps
//============================================================================
// 1D CNN Top Module — LUT RAM (Distributed RAM) Only Variant
//
// All weights and biases are stored in Distributed (LUT) RAM inside
// sub-modules. No Block RAM used anywhere in the design.
//
// Architecture (same CNN, different memory):
//   Conv1 (1→4, k=5) → ReLU → MaxPool(4)   [conv_pool_1d_lutram]
//   Conv2 (4→8, k=3) → ReLU → MaxPool(4)   [conv_pool_1d_lutram]
//   FC1   (384→32)   → ReLU                 [layer_seq_lutram]
//   FC2   (32→10)    → logits               [layer_seq_lutram]
//============================================================================
module cnn_top_lutram #(
    // ---- Conv1 ----
    ...
    // Weight/bias file paths (all stored as LUT RAM inside sub-modules)
    ...
    // ==================================================================
    //  Conv1 + Pool1 (LUT RAM weights, biases, conv buffer)
    // ==================================================================
    conv_pool_1d_lutram #(
        .IN_LEN      (CONV1_IN_LEN),
        .IN_CH       (CONV1_IN_CH),
    ...
    // ==================================================================
    //  Conv2 + Pool2 (LUT RAM weights, biases, conv buffer)
    // ==================================================================
    conv_pool_1d_lutram #(
    ...
    // ==================================================================
    //  FC1: 384 → 32, ReLU (LUT RAM weights and biases)
    // ==================================================================
    layer_seq_lutram #(
        .NUM_NEURONS       (FC1_OUT),
    ...
    // ==================================================================
    //  FC2: 32 → 10, no activation (LUT RAM weights and biases)
    // ==================================================================
    layer_seq_lutram #(
        .NUM_NEURONS       (FC2_OUT),
