`timescale 1ns / 1ps
//============================================================================
// Parametric 2D Max-Pooling Module (synthesis-safe — no division/modulo)
//
...
// - Direct counters avoid combinational division
//
// Memory layout (flat):
...
module maxpool2d #(
    ...
    // Direct counters — no division needed
    reg [31:0] out_row_cnt;    // 0 .. OUT_H-1
    reg [31:0] out_col_cnt;    // 0 .. OUT_W-1
    reg [31:0] pool_r_cnt;     // 0 .. POOL_H-1
    reg [31:0] pool_c_cnt;     // 0 .. POOL_W-1
    reg [31:0] pos_counter;    // flat output position (for indexing data_out)
    reg [31:0] pool_counter;   // flat pool element (for end-of-window check)

    // Input coordinates derived from counters (multiply-by-constant only)
    wire [31:0] in_row;
    wire [31:0] in_col;
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            case (state)
                ...
                S_COMPARE: begin
                    // Compare all channels in parallel
