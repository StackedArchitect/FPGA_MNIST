`timescale 1ns / 1ps
//==============================================================================
// cnn1d_synth_top_lutram.sv — Synthesizable top wrapper (LUT RAM Only Variant)
//
// All weights, biases, and the input image ROM use Distributed (LUT) RAM.
// No Block RAM used anywhere in the design.
//
// Adjust $readmemh paths relative to your Vivado project root (.xpr location).
//==============================================================================

module cnn1d_synth_top_lutram (
    input  wire        clk,
    input  wire        rstn,
    ...
    // -------------------------------------------------------------------------
    // Input image ROM — Distributed (LUT) RAM
    // -------------------------------------------------------------------------
    (* ram_style = "distributed" *) reg signed [31:0] pixel_in [0:783];
    ...
    // -------------------------------------------------------------------------
    // DUT — all weights/biases loaded from .mem files via LUT RAM ROM
    // -------------------------------------------------------------------------
    ...
    // clock period (WNS = −2.399 ns on 4 endpoints = pred_out[3:0]).
    //
