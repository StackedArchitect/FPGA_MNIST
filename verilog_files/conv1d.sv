...
module conv1d #(
    ...
    // Direct 2D tap counters — eliminates combinational division by KERNEL_SIZE
    ...
    // Input data index: channel * IN_LEN + pos + k  (no division)
    wire [31:0] data_idx;
    assign data_idx = ch_cnt * IN_LEN + pos_counter + k_cnt;
    assign cur_data = data_in[data_idx];

    // Tap index for weight array: ch_cnt * KERNEL_SIZE + k_cnt (adder, no division)
    wire [31:0] tap_idx;
    assign tap_idx = ch_cnt * KERNEL_SIZE + k_cnt;

    // Weight index for each filter: filter * TAP_COUNT + tap_idx
    genvar f;
    generate
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            case (state)
                ...
                // ---- COMPUTE: MAC — advance (k -> ch) counters, no division ----
                S_COMPUTE: begin
                    for (i = 0; i < OUT_CH; i = i + 1)
