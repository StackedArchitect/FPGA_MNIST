...
//   Stage 2 (posedge): Q16.16 multiply    → registered p2_mult
//   Stage 3 (posedge): accumulate p2_mult  → acc register
//
...
module layer_seq #(
    ...
    //  Layout: neuron_0 weights [0..W], neuron_1 weights [0..W], ...
    //  Same row-major order as $readmemh fills a 2D array [N][W+1].
    // ================================================================
    ...
    localparam S_DRAIN = 3'd3;   // Drain 2-stage pipeline (2 cycles)
    ...
    //  Both have 1-cycle latency, keeping the pipeline aligned.
    // ================================================================
    ...
    // Breaks the critical path: cur_weight × cur_data (32×40 for FC2)
    // from ~20 ns down to ~10 ns per stage.
    ...
    //   feeding = 1 when a valid address is being presented to stage 1
    //   pipe_s1_valid: stage 1 output is valid (1 cycle after feeding)
    //   pipe_s2_valid: p2_mult is valid      (2 cycles after feeding)
    wire feeding;
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            case (state)
                ...
                // ---- Accumulate pipeline output (p2_mult), issue
                ...
                // ---- Drain: flush the 2-stage pipeline.
                //      2 cycles to collect the last 2 products. ----
                S_DRAIN: begin
                    if (pipe_s2_valid)
                        acc <= acc + p2_mult;
